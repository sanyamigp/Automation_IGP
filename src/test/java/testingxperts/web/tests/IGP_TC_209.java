...
public class IGP_TC_209 extends KeywordUtil {
	String stepInfo = "";
	int retryCount = getIntValue("retryCount");
	static int retryingNumber = 1;

	@Test(testName = "IGP_TC_209", 
			groups = {"Payment" }, 
			description = "Net Banking: ICICI Bank-: Ensure that ICICI bank in Net Banking section")
	public void test() throws Throwable {
		try {
			...
			stepInfo="Verify selecting ICICI bank and click on payment user should navigate to ICICI netbanking page";
			logStep(stepInfo);
			PaymentPage.makePaymentWithBank(Bank.ICICI_BANK);
			pause(6000);
			
			String currentUrl =getCurrentUrl();
			verifyStep(currentUrl.contains("shopping.icicibank.com"), stepInfo);
			
			
			String elementSShot = takeScreenshot(getDriver(),"PaymentBANK_ICICI");
