...
public class IGP_TC_214 extends KeywordUtil{
	String stepInfo="";
	...
	@Test(
			testName="IGP_TC_214",
			groups={"Payment"}, 
			description="Wallets: Paytm - Ensure that Paytm  option in Wallets section."
			)
	public void test() throws Throwable {
		try{
			...
			*/
			...
			stepInfo="Verify when user select Payment option Wallets Paytm and click make payment. User should navigate to PAYTM page";
			logStep(stepInfo);
			PaymentPage.makePaymentWithWallet(Wallets.PAYTM);
			pause(6000);
			String currentUrl =getCurrentUrl();
			verifyStep(currentUrl.contains("secure.paytm.in"), stepInfo);
			
			String elementSShot = takeScreenshot(getDriver(),"PaymentBANK_PAYTM");
