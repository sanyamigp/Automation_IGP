...
public class IGP_TC_210 extends KeywordUtil {
	String stepInfo = "";
	int retryCount = getIntValue("retryCount");
	static int retryingNumber = 1;

	@Test(testName = "IGP_TC_210", 
			groups = {"Payment" }, 
			description = "Net Banking: Citi Bank-: Ensure that citi bank in Net Banking section")
	public void test() throws Throwable {
		try {
			...
			stepInfo="Verify selecting CITY bank and click on payment user should navigate to CITY netbanking page";
			logStep(stepInfo);
			PaymentPage.makePaymentWithBank(Bank.CITI_BANK);
			pause(6000);
