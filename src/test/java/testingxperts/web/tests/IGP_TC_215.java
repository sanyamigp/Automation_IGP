...
public class IGP_TC_215 extends KeywordUtil{
	String stepInfo="";
	...
	@Test(
			testName="IGP_TC_215",
			groups={"Payment"}, 
			description="Wallets: Buddy- : Ensure that  Buddy  option in Wallets section."
			)
	public void test() throws Throwable {
		try{
			...
			*/
			...
			PaymentPage.clickPaymentOption(PaymentPage.PaymentOptions.WALLETS);
			stepInfo="Verify WALLETS option";
			logStep(stepInfo);
			verifyStep(
			PaymentPage.verifyPaymentOptionIsDisplayed(PaymentPage.PaymentOptions.WALLETS),
			stepInfo);
			pause(1000);
			
			stepInfo="Verify when user select Payment option Wallets BUDDY and click make payment. User should navigate to BUDDY page";
			logStep(stepInfo);
			PaymentPage.makePaymentWithWallet(Wallets.BUDDY);
			pause(6000);
			String currentUrl =getCurrentUrl();
			verifyStep(currentUrl.contains("erupee.sbi.co.in"), stepInfo);
			
			String elementSShot = takeScreenshot(getDriver(),"PaymentBANK_BUDDY");
