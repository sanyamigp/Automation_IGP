...
public class IGP_TC_208 extends KeywordUtil {
	String stepInfo = "";
	int retryCount = getIntValue("retryCount");
	static int retryingNumber = 1;

	@Test(testName = "IGP_TC_208", 
			groups = {"Payment" }, 
			description = "Net Banking: Axis Bank -: Ensure that Axis bank in Net Banking section")
	public void test() throws Throwable {
		try {
			...
			stepInfo="Verify selecting AXIS bank and click on payment user should navigate to Axis netbanking page";
			logStep(stepInfo);
			PaymentPage.makePaymentWithBank(Bank.AXIS_BANK);
			pause(6000);
			
			String currentUrl =getCurrentUrl();
			verifyStep(currentUrl.contains("retail.axisbank.co.in"), stepInfo);
			
			
			String elementSShot = takeScreenshot(getDriver(),"PaymentBANK_AXIS");
