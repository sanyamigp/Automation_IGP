...
public class IGP_TC_207 extends KeywordUtil {
	String stepInfo = "";
	int retryCount = getIntValue("retryCount");
	static int retryingNumber = 1;

	@Test(testName = "IGP_TC_207", 
			groups = {"Payment" }, 
			description = "Net Banking: HDFC Bank -: Ensure that HDFC bank in Net Banking section")
	public void test() throws Throwable {
		try {
			...
			stepInfo="Verify selecting HDFC bank and click on payment user should navigate to hdfc netbanking page";
			PaymentPage.makePaymentWithBank(Bank.HDFC_BANK);
			pause(6000);
			
			String currentUrl =getCurrentUrl();
			verifyStep(currentUrl.contains("netbanking.hdfcbank.com"), stepInfo);
			
			
			String elementSShot = takeScreenshot(getDriver(),"PaymentBANK_HDFC");
